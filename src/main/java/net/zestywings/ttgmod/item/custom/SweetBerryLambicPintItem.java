...
public class SweetBerryLambicPintItem extends Item {
    private static final int MAX_USE_TIME = 40;

    public SweetBerryLambicPintItem(Settings settings) {
        super(settings);
    }
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        if (!world.isClient) {
            user.addStatusEffect(new StatusEffectInstance(ModEffects.THORNY, 3600));
        ...
        if (stack.isEmpty()) {
            return new ItemStack(ModItems.EMPTY_GLASS_PINT);
        } else {
            if (user instanceof PlayerEntity && !((PlayerEntity) user).getAbilities().creativeMode) {
                ItemStack itemStack = new ItemStack(ModItems.EMPTY_GLASS_PINT);
